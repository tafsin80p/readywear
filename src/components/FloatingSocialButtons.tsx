...
export function FloatingSocialButtons({ whatsappUrl, messengerUrl }: FloatingSocialButtonsProps) {
  return (
    <div className="fixed right-4 top-1/2 -translate-y-1/2 flex flex-col gap-3 md:gap-4 z-50">
      <a 
        href={whatsappUrl || "#"} 
        target={whatsappUrl ? "_blank" : "_self"} 
        rel="noopener noreferrer"
        className="relative group flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full bg-green-500 hover:bg-green-600 transition-colors shadow-lg hover:shadow-xl hover:scale-110 duration-300"
      >
        ...
        <svg className="w-6 h-6 md:w-8 md:h-8 text-white z-10" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a 
        href={messengerUrl || "#"} 
        target={messengerUrl ? "_blank" : "_self"} 
        rel="noopener noreferrer"
        className="relative group flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-[#00c6ff] to-[#0072ff] hover:brightness-110 transition-all shadow-lg hover:shadow-xl hover:scale-110 duration-300"
      >
        {/* Ripple animation layers */}
