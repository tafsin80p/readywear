    ...
    fetch('/api/admin/categories')
      ...
      .then(data => {
        if (data.success && data.categories) {
          ...
          // Ensure formData.category is set to a valid initial value if not already
          setFormData(prev => {
            if (!prev.category && data.categories.length > 0) {
              return { ...prev, category: data.categories[0].slug };
            }
            return prev;
          });
        }
    ...
    <form onSubmit={handleSubmit} className="p-6 md:p-8 w-full animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 bg-white p-4 md:p-6 rounded-2xl border border-gray-100 shadow-sm sticky top-0 z-30">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight">Add New Product</h1>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Plus className="w-5 h-5" /> Add Product</>}
          </button>
        </div>
      </div>
